"""
This script uses Random Forests with mass balance to estmate energy production
...
def create_lagged_features(df):
    """Creates a lagged mass balance feature for the given dataframe."""
    df_lagged = df.copy()
...
# Define features and prepare development data (precipitation removed)
...
if n_splits < 1:
    raise ValueError("Not enough data in development set for cross-validation after lagging.")
...
try:
    ...
    plt.title('RF Hydropower Forecast using Lag Time and No Precipitation')
    plt.grid(True)
    plt.legend()
    plt.show()
